...
const ModelManage: React.FC = () => {
  ...
  const uploadSave = async () => {
    if (assetName == '' || assetDescription == '' || files.length == 0 || tags.filter((tag) => tag.isSelected == true).length == 0) {
      await confirm('Name, Description, uploading files, and min. 1 tag is requiered', false, dispatch);
